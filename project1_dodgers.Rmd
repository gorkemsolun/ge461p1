...
To observe whether different variables increase attendance, Chi-squared tests and Welch’s Two Sample t-tests were conducted on variables and attendance. To make sure of the result of analysis, we define a linear regression model with all variables to compare with different models afterwards.
...
```
The results suggest that cap promotions have a near to zero impact on attendance. The test showed a disconnection between cap promotions and attendance numbers. Our p-value is more than 0.05, so we are confident that cap promotions is not related to attendance.
...
```
Like the graph, the results suggest that being day or night has no apparent impact on attendance. The test showed a disconnection between being day or night and attendance numbers. Our p-value is more than 0.05, meaning we are confident that being day or night is not related to attendance.
...
In contrast to the Chi-squared test, the results suggest that skies have a low impact on attendance. The test showed a disconnection between sky and attendance numbers. Our p-value is more than 0.05, which means we are confident that sky is unrelated to attendance.
...
```{r}
model1 <- lm(attend ~ month + day_of_week + bobblehead, data = events)
summary(model1)
```
...
We test the null hypothesis, "Giving bobblehead has no effect on attendance," in the summary table along the lines of the bobbleheadYES. If the p-value on the same line is significant (> 0.05), then the null and the data are consistent. We reject the null if the p-value is small because the data contradicts the null. We reject the null since the p-value is very small. Consequently, we draw the conclusion that bobbleheads have an impact on attendance, although a favorable one given the estimate's positive. On average, we anticipate a rise in attendance.
...
Welch Two Sample t-tests between some of the requirements and attendance were conducted after running the first model to check whether there is a correlation between the two in order to expedite the criteria removal process. The accuracy of the model should theoretically increase if a criterion is eliminated if it does not correspond with attendance.

## Day or Night

The first thing to be checked was attendance and if the match was taking place during the day or night.
...
At a significance level of 0.05, the test appears to indicate that there is no correlation between the game's time of day and attendance.

```{r}
model2 <- lm(formula = attend ~ month + day + day_of_week + opponent + temp + skies #+ day_night
             + cap + shirt + fireworks + bobblehead, data = train)
...
We also see a drop in the MAE of the predictions when the day/night criteria are removed from the initial regression model.

## Shirt

We also tested shirt promotions.

```{r}
model3 <- lm(formula = attend ~ month + day + day_of_week + opponent + temp + skies #+ day_night
             + cap #+ shirt
             + fireworks + bobblehead, data = train)

prediction3 <- predict(model3,test)

mean(abs(prediction3-test$attend))
```
We can see a drop but it is very small so, it may be related.

## Caps

We also tested cap promotions.
...
```{r}
model4 <- lm(formula = attend ~ month + day + day_of_week + opponent + temp + skies 
             #+ day_night + cap + shirt
             + fireworks + bobblehead, data = train)
...
```{r}
model5 <- lm(formula = attend ~ month + day + day_of_week + opponent + temp 
...
It is found that eliminating uncorrelated criteria from the model lowers the MAE, indicating a higher level of accuracy for the model until skies. The sky requirements, however, defy this reasoning. This time we see a rise so we should include skies.

## Fireworks

We also tested fireworks.
...
There is little chance that the presence of fireworks will affect attendance, as indicated by the p-value of 0.98.

```{r}
model6 <- lm(formula = attend ~ month + day + day_of_week + opponent + temp 
             + skies #+ day_night + cap + shirt + fireworks
             + bobblehead, data = train)
...
In contrast to t-test and similar to skies, this time we see a rise so we should include fireworks.

## Final Model

By removing our unrelated values, we get a final model.

TODO: WE MAY NEED TO DİSCUSS OUR PReVİOUS REULTS HERE AND SAY WHAT İS ADDED
...
The final model yields the best result. Looking at the p-values, we can see here bobblehead has a positive effect. 

The Mean Absolute Error calculation.

```{r}
finalprediction <- predict(final,test)

mean(abs(finalprediction-test$attend))
```

Our models prediction is better cansidering the MAE calculation.

Bar plot of the final model prediction.

```{r}
barplot(finalprediction-test$attend,col='red')
```
...
A common guideline is that a VIF above 5 suggests potential issues with multicollinearity. Our model has less than 5.

Given a 90% confidence interval, these many fans are expected to be drawn alone by a bobblehead promotion.

```{r}
confint(final, level = 0.90)["bobbleheadYES",]
```

```{r}
coef(final)["bobbleheadYES"]
```

With the best multiple regression model to predict attendance, including bobblehead promotions, we can see that bobblehead promotions contribute positively to the games' attendance, considering the tests conducted earlier.
