...
events %>% 
  ...
  pull(avgattend) %>% chisq.test()
```
In contrast to the graph, the results suggest that being skies have a impact on attendance. The test showed a connection between sky and attendance numbers. Our p-value is near zero meaning we are confident that sky is related to attendance.
...
The graph suggests a difference, particularly around 22-23 degrees. We can see higher averages of attendance. Therefore, we will consider skies relations in attendance.
